...
class ResNet(nn.Module):
    def __init__(self, config, args, device):
        ...
        layer1 = tran_2.TransformerEncoderLayer(
            d_model=128, nhead=8, dim_feedforward=256, dropout=0.1,batch_first=True)
        self.encoder1 = tran_2.TransformerEncoder(layer1, num_layers=2)
        layer2 = tran_2.TransformerDecoderLayer(
            d_model=256, nhead=8, dim_feedforward=128, dropout=0.1,batch_first=True)
        self.encoder2 = tran_2.TransformerDecoder(layer2, num_layers=2)
    ...
    def forward(self, samples_x, samples_y0,samples_yt, info, diffusion_step):
        ...
        # z_mix = self.CNN(x)
        # if self.training == True:
        #     z_mix = self.dec7(mask * x) + (1 - mask) * y0
        # else:
        #     z_mix = self.dec7(mask * x)
        if self.training == True:
        ...
        #
        # z_k = self.dec6(y.permute(0,2,1)).permute(0,2,1)

        z_ar = self.armodel(x)
        c = self.dec6(torch.cat((z_mix,z_ar),dim=2))
        y = yt + diffussion_emb_y
        ...
        for layer in self.residual_layers:
            triplets_y = yt + diffussion_emb_y
            yt, skip_connection = layer(triplets_x, triplets_y)
