...
class TemporalBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return out, self.relu(out.permute(0,2,1) + res)
...
class ResNet(nn.Module):
    def __init__(self, config, args, device):
        ...
        layer2 = tran.TransformerEncoderLayer(
            d_model=256, nhead=8, dim_feedforward=128, dropout=0.1)
        self.encoder2 = tran.TransformerEncoder(layer2, num_layers=2)
    ...
    def forward(self, samples_x, samples_y, info, diffusion_step):
        ...
        x = triplets_x
        y= triplets_y
        mask_x = torch.randn_like(x)
        mask_y = mask_x[:,0:30,:]
        z_mix = self.CNN(x)
        if self.training == True:
            z_mix = (mask_x * z_mix) + self.dec7((1 - mask_y) * y)
        else:
            z_mix = mask_x * z_mix
        z_ar = self.armodel(x)
        c = torch.cat((z_mix,z_ar[1]),dim=2)
        c = self.dec5(c)
        y = y+diffussion_emb_y
        z_k = self.encoder1(y)
        z_k = self.dec6(z_k.permute(0,2,1)).permute(0,2,1)
        c = z_k + c
        output = self.encoder2(c)
