...
#         super().__init__()
#         self.device = device
#         self.register_buffer('Pos_embedding', self._build_embedding(max_len + 1, d_model), persistent=False)
#
#     def forward(self, m):
#         return self.Pos_embedding[m]
#
#     def _build_embedding(self, t, d_model):
#         pe = torch.zeros(t, d_model).to(self.device)
#         position = torch.arange(t).unsqueeze(1).to(self.device)
...
class PositionalEncoding(nn.Module):
    def __init__(self, dim, dropout, max_len=100):
        # dim: 词嵌入的维度
...
class ResNet(nn.Module):
    def __init__(self, config, args, device):
        ...
        self.emb_text = nn.Linear(1, self.channels).to(device)
        ...
        self.pos_emb = PositionalEncoding(self.channels,0.2)
    ...
    def forward(self, samples_x, samples_y0,samples_yt, info, diffusion_step):
        ...
        triplets_y0 = self.pos_emb(self.emb_text(samples_y0[:,0].unsqueeze(-1))) * samples_y0[:, 1].unsqueeze(-1)
        triplets_yt = self.pos_emb(self.emb_text(samples_yt[:, 0].unsqueeze(-1))) * samples_y0[:, 1].unsqueeze(-1)
        # diffussion_emb_y = diffusion_emb[:, : int(self.size_y)].reshape(diffusion_emb.shape[0],int(self.size_y//10),10,128) * samples_y0[:, 1].unsqueeze(-1)
        diffussion_emb_y = diffusion_emb[:, : self.size_y] * samples_y0[:, 1].reshape(samples_y0[:, 1].shape[0],samples_y0[:, 1].shape[1]*samples_y0[:, 1].shape[2]).unsqueeze(-1)
        x=triplets_x#32,60,128
        yt=triplets_yt#32,3,10,128
        yt=triplets_yt.reshape(diffusion_emb.shape[0], self.size_y, 128)
        ...
        # z_mix = self.CNN(x)
        # if self.training == True:
        #     z_mix = self.dec7(mask * z_mix) + (1 - mask) * y0
        # else:
        #     z_mix = self.dec7(mask * z_mix)
        ...
        output = output.reshape(output.shape[0],3,10)
        return output # 32,30
