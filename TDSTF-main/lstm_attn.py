...
from tran import Transformer as transformer
from models.Transformer import Model as tran
...
class ResNet(nn.Module):
    def __init__(self, config, args, device):
        ...
        self.CNNx = Bottleneck1D(128, 64, 1)
        self.CNNy = Bottleneck1D(128, 128, 1)
        ...
        self.residual_layers = nn.ModuleList([
            Triplet_cor(config, args, device)
            for _ in range(config['layers'])])
        self.attn = tran(args).to(device)
    ...
    def forward(self, samples_x, samples_y, info, diffusion_step):
        ...
        # 早期层融合
        # triplets_x = self.CNNx(triplets_x)
        # triplets_y = self.CNNy(triplets_y)

        triplets_x = triplets_x + diffusion_emb * samples_x[:, 3].unsqueeze(-1)
        triplets_y = triplets_y + diffussion_emb_y

        triplets_x,_ = self.lstm(triplets_x)
        triplets_y,_ = self.lstm(triplets_y)
        output = self.attn(triplets_x,triplets_y)


        output = self.CNNx(output)
...
class Triplet_cor(nn.Module):
    def __init__(self, config, args, device):
        ...
        # self.attn = tran(args).to(device)
        # self.attn = informer(args)
        # self.attn = transformer(d_model=self.channels, nhead=8, num_encoder_layers=config['enl'],num_decoder_layers=config['del'], dim_feedforward=256, dropout=0.1,activation='gelu', batch_first=True, device=device)
