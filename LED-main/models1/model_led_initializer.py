...
from models1.layers import MLP, social_transformer, st_encoder

class LEDInitializer(nn.Module):
    def __init__(self, t_h: int=60, d_h: int=1, t_f: int=30, d_f: int=1, k_pred: int=20):
        '''
        Parameters
        ----
    ...
    def forward(self, x, mask=None):
        ...
        guess_mean = self.mean_decoder(mean_total).contiguous().view(-1, self.fut_len, 1)
        #var
        ...
        guess_var = self.var_decoder(var_total).reshape(x.size(0), self.n, self.fut_len, 1)
