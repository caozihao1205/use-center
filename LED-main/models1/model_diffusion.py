...
from models1.layers import PositionalEncoding, ConcatSquashLinear
...
class st_encoder(nn.Module):
	...
	def forward(self, X):
		...
		state_x = state_x.squeeze(0)#1,32,256

		return state_x#32,256


class social_transformer(nn.Module):
	def __init__(self):
		super(social_transformer, self).__init__()
		self.encode_past = nn.Linear(60*4, 256, bias=False)
...
class TransformerDenoisingModel(Module):
	...
	def __init__(self, context_dim=256, tf_layer=2):
		...
		self.pos_emb = PositionalEncoding(d_model=2*context_dim, dropout=0.1, max_len=34)
		self.concat1 = ConcatSquashLinear(1, 2*context_dim, context_dim+3)
		...
		self.linear = ConcatSquashLinear(context_dim//2, 1, context_dim+3)
	...
	def generate_accelerate(self, x, beta, context, mask):
		...
		# x: 11, 10, 20, 2
		# ctx_emb: 11, 10, 1, 259
		x = self.concat1.batch_generate(ctx_emb, x).contiguous().view(-1, 30, 512)
		# x: 110, 20, 512
		final_emb = x.permute(1, 0, 2)
		final_emb = self.pos_emb(final_emb)
		
		trans = self.transformer_encoder(final_emb).permute(1, 0, 2).contiguous().view(-1, 10, 30, 512)
		# trans: 11, 10, 20, 512
