...
for file in os.listdir('preprocess/data/first'):
    sr, ir = pickle.load(open('preprocess/data/first/' + file, 'rb'))
    samples += sr
    info += ir
...
pickle.dump([mean, std], open('preprocess/data/mean_std.pkl','wb'))
pickle.dump([data_sets[0], pd.DataFrame(info_sets[0]), data_sets[1], pd.DataFrame(info_sets[1]), data_sets[2], pd.DataFrame(info_sets[2])], open('preprocess/data/dataset.pkl','wb'))
