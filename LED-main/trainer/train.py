...
from models1.model_led_initializer import LEDInitializer as InitializationModel
from models1.model_diffusion import TransformerDenoisingModel as CoreDenoisingModel
...
class Trainer:
    def __init__(self, config):
        ...
        size = 60
        self.train_loader, self.valid_loader, self.test_loader = get_dataloader(data_path, var_path, size)
        ...
        self.model_initializer = InitializationModel().cuda()
        ...
        # ------------------------- prepare logs -------------------------
        self.log = open(os.path.join(self.cfg.log_dir, 'log1.txt'), 'a+')
        ...
        self.temporal_reweight = torch.FloatTensor([31 - i for i in range(1, 31)]).cuda().unsqueeze(0).unsqueeze(0) / 10
    ...
    def _train_single_epoch(self, epoch):
        ...
        for data in self.train_loader:
            ...
            samples_x, samples_y, info = self.process(data)
            x_ori = samples_x[:, 0:4].permute(0, 2, 1)
            x_mask = samples_x[:, 3].unsqueeze(-1)
            y_ori = samples_y[:, 2].unsqueeze(1).permute(0, 2, 1)
            y_mask = samples_y[:, 3].unsqueeze(-1)
            sample_prediction, mean_estimation, variance_estimation = self.model_initializer(x_ori, None)
            ...
            generated_y = self.p_sample_loop_accelerate(x_ori, None, loc)

            loss_dist = ((generated_y - y_ori.unsqueeze(dim=1)).norm(p=2, dim=-1)
                         *
                         self.temporal_reweight
                         ).mean(dim=-1).min(dim=1)[0].mean()
            loss_uncertainty = (torch.exp(-variance_estimation)
                                *
                                (generated_y - y_ori.unsqueeze(dim=1)).norm(p=2, dim=-1).mean(dim=(1, 2))
                                +
                                variance_estimation
                                ).mean()
