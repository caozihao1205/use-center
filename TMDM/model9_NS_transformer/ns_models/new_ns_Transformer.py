...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.dec_embedding = ADataEmbedding(configs.dropout)
    ...
    def KL_loss_normal(self, posterior_mean, posterior_logvar):
        KL = -0.5 * torch.mean(1 - posterior_mean ** 2 + posterior_logvar -
                               torch.exp(posterior_logvar), dim=1)#32,512
        return torch.mean(KL)#1
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        dec_out=dec_out
        std_enc=std_enc.unsqueeze(-1)
        mean_enc=mean_enc.unsqueeze(-1)
        # De-normalization
        ...
        if self.output_attention:
            ...
            return dec_out, dec_out, KL_z, z_sample  # [B, L, D]  dec_out[:, -self.pred_len:, :]
